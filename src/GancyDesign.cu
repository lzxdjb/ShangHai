...
void MoreCrazy(uint8_t *query, uint8_t *key, float *LUT, float *store)
{ 
    ...
    // for(int i = 0 ; i < 1 ; i ++)
    {
        ...
        int QOffset = StreamNumber * qlen * dim * FancyStreamWorkLoad ;
        int KOffset = StreamNumber * klen * dim * FancyStreamWorkLoad;

        checkCudaErrors(cudaMemcpyAsync(&d_query[QOffset], &query[QOffset], (qlen * dim) * sizeof(uint8_t) * FancyStreamWorkLoad, cudaMemcpyHostToDevice, stream[StreamNumber % StreamModulnStreams]));

        // int KOffset = OutOffset * klen * dim;
        checkCudaErrors(cudaMemcpyAsync(&d_key[KOffset], &key[KOffset], (klen * dim)  * sizeof(uint8_t) * FancyStreamWorkLoad, cudaMemcpyHostToDevice, stream[StreamNumber % StreamModulnStreams]));
        ...
        int StoreOffset = StreamNumber * qlen * klen * FancyStreamWorkLoad;
        checkCudaErrors(cudaMemcpyAsync(&store[StoreOffset], &d_store[StoreOffset], (qlen * klen)*sizeof(float) * FancyStreamWorkLoad, cudaMemcpyDeviceToHost, stream[StreamNumber % StreamModulnStreams]));
    ...
    ///// End Time Record
    printf("Time for Crazy execute time (ms): %f\n", ms);
