...
int reader(int* ndCnt, int* edgCnt, int* lutCnt, int* regCnt, int*srcCnt,
    ...
    while (fgets(line, maxLineChars, file) != NULL) {
        ...
        if (sscanf(line, "%s INPUT %s", nodeID, nodeID) == 2) {                         // Process INPUT
            ...
            typIDs[id] = *srcCnt;
            printf("nodeID[%d] is pushing [%d] to offsets\n", id, offsCntr);
...
int main() {
    // (int* ndCnt,int* edgCnt,int* lutCnt,int* regCnt,int* srcCnt, int* srcIDs,int* offs,int* csr,int* typeIDs, int* regInd, int* lutOffs, int* lutBts)
    ...
    srcIDs = srcIDs_vec.data();
    offsets = offs_vec.data();
    csr = csr_vec.data();
    lutOffsets = lutOffs_vec.data();
    lutBits = lutBts_vec.data();
    regIndicator = regInd_vec.data();
    typeIDs = typeIDs_vec.data();
